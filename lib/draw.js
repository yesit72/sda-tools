import { FORMATS, INK, MUTED, DOTS, W, H, PT, LAYOUT } from './formats.js';
...
export function drawNewsletter(ctx, data, assets) {
  ...
  const bodyTop = y + 24;
  let size = (photos.length ? 17 : 19) * PT, lh = 1.5, gap = 13 * PT;
  const fits = (s) => {
  ...
  while (size > 13 * PT && !fits(size)) size -= PT * 0.5;
